...
export default async function Search() {
 ...
 const categories = ['men', 'Electronics', 'Clothing', 'Books'] // Replace with actual data
    ...
    <form action='/search' method='GET' className='flex  items-stretch h-10 '>
      <Select name='category'>
        <SelectTrigger className='w-auto h-full dark:border-gray-200 bg-gray-100 text-black border-r  rounded-r-none rounded-l-md rtl:rounded-r-md rtl:rounded-l-none  '>
          <SelectValue placeholder=/*{t('Header.All')*/ 'All'/>
        </SelectTrigger>
        <SelectContent position='popper'>
          <SelectItem value='all'>All</SelectItem>{/*{t('Header.All')*/}
      ...
      <Input
        ...
        placeholder=  {`Search Site ${APP_NAME}`} /*{t('Header.Search Site', { name })}*/
        name='q'
        type='search'
      />
      <button
